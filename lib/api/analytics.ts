...
export async function getUserAnalytics(userId: string, days = 30) {
  ...
  const totalAiRequests = usage.reduce((sum: number, u: { aiRequests: number }) => sum + u.aiRequests, 0);
  const totalPrompts = usage.reduce((sum: number, u: { promptsUsed: number }) => sum + u.promptsUsed, 0);
  const totalExports = usage.reduce((sum: number, u: { exports: number }) => sum + u.exports, 0);
...
export async function getAdminAnalytics(startDate?: Date, endDate?: Date) {
  ...
  const totalAiRequests = dailyAnalytics.reduce((s: number, d: { aiRequests: number }) => s + d.aiRequests, 0);
  const totalPrompts = dailyAnalytics.reduce((s: number, d: { totalPrompts: number }) => s + d.totalPrompts, 0);
  const favorites = dailyAnalytics.reduce((s: number, d: { totalFavorites: number }) => s + d.totalFavorites, 0);
  const avgResponseTime = dailyAnalytics.reduce((s: number, d: { avgResponseTime: number | null }) => s + (d.avgResponseTime || 0), 0) / (dailyAnalytics.length || 1);
...
export async function getUsageStats(userId: string) {
  ...
  return {
    ...
    monthly: {
      aiRequests: monthlyUsage.reduce((s: number, u: { aiRequests: number }) => s + u.aiRequests, 0),
      promptsUsed: monthlyUsage.reduce((s: number, u: { promptsUsed: number }) => s + u.promptsUsed, 0),
      exports: monthlyUsage.reduce((s: number, u: { exports: number }) => s + u.exports, 0),
    },
    total: {
